...
if (is_array(get_field('profiles'))) {
    ...
    // going into contact item data to grab nested data
        $profile_data['scheduling_link'] = esc_url($schedule_data[0]['schedule_appointment_link']['url'] ?? '');
        $profile_data['scheduling_text'] = esc_html($schedule_data[0]['schedule_appointment_link']['title'] ?? '');
